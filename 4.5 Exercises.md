...
**Exercise 4.33.** The rational numbers (often written Q) are the set of fractions a / b , where
a and b are integers (but b = 0). For example, 1/2 , -6/11 , and 50/3 are all rational. Any integer,
...
Yes.
(b) Is it a group using ordinary multiplication as the operation?
No, because 0(zero) doesn't an inverse.
(c) Call Q+ the positive rational numbers (only those greater than zero). Is Q+ a group
under ordinary addition?
No, because there are neither inverse elements, nor identity element.
(d) Is Q+ a group under ordinary multiplication?
Yes, it is.
...
No, because there is no identity element. The group is not closed, e.g. -5+5 = 0 is not en element of the group.
(f) Is Q* a group under ordinary multiplication?
Yes, it is.
(g) Why are groups like Q, Q+, and Q* difficult to visualize using multiplication tables
and Cayley diagrams?
Because they are infinite. More over between each pair of rational numbers there are infinite number of other rational numbers. 